...
export class MatchesGateway extends EventsGateway {
  ...
  handleDisconnect(client) {
    ...
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('player_left', userID);
    this.matchesPlayers[matchID].splice(
      this.matchesPlayers[matchID].indexOf(userID),
      1,
    );
  ...
  async emitNewCompleteThrow(
    ...
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('new_complete_throw', { userID, newThrow });
  }

  async emitNewPartialThrow(
    userID: string,
    matchID: string,
    newThrow: Throw,
  ): Promise<void> {
    ...
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('new_partial_throw', { userID, newThrow });
  }

  async emitPartialThrowUndo(userID: string, matchID: string): Promise<void> {
    console.log('Emitting partial throw undo');
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('partial_throw_undo');
  }

  async emitCompleteThrowUndo(userID: string, matchID: string): Promise<void> {
    console.log('Emitting complete throw undo');
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('complete_throw_undo');
  }
  ...
  async emitLegWon(
    ...
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('leg_won', { nickname: user.nickname, leg, set });
  }

  async emitMatchWon(user: User, matchID: string): Promise<void> {
    this.server
      .to(MatchesGateway.getRoomID(matchID))
      .emit('match_won', { nickname: user.nickname });
  }
}
