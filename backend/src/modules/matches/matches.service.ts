...
export class MatchesService implements NotificationsProvider {
  ...
  async updateMatchPlayersAndRequests(match: Match): Promise<Match> {
    return this.matchModel.findByIdAndUpdate(match._id, {
      $set: {
  ...
  async emitPartialThrowUndo(userID: string, matchID: string) {
    await this.matchesGateway.emitPartialThrowUndo(userID, matchID);
  }

  async emitCompleteThrowUndo(userID: string, matchID: string) {
    await this.matchesGateway.emitCompleteThrowUndo(userID, matchID);
  }
